...
class StataParser(object):
    def __init__(self, encoding):
        ...
        self.DTYPE_MAP = \
            dict(
                list(zip(list(range(1, 245)), ['a' + str(i) for i in range(1, 245)])) +
                [
                    (251, np.int16),
                    (252, np.int32),
                    (253, np.int64),
...
class StataReader(StataParser):
    ...
    def _col_size(self, k=None):
        ...
        if len(self.col_sizes) == 0:
            self.col_sizes = [self._calcsize(x) for x in self.typlist]
        if k is None:
    ...
    def _next(self):
        ...
        if self.has_string_data:
            ...
            return [self._unpack(typlist[i],
                                              self.path_or_buf.read(self._col_size(i))) for i in range(self.nvar)]
    ...
    def data(self, convert_dates=True, convert_categoricals=True, index=None):
        ...
        if convert_dates:
            cols = np.where([x in _date_formats for x in self.fmtlist])[0]
            for i in cols:
                col = data.columns[i]
                data[col] = data[col].apply(_stata_elapsed_date_to_datetime, args=(self.fmtlist[i],))

        if convert_categoricals:
            cols = np.where([x in iter(self.value_label_dict.keys()) for x in self.lbllist])[0]
            for i in cols:
                col = data.columns[i]
                labeled_data = np.copy(data[col])
                labeled_data = labeled_data.astype(object)
                for k, v in self.value_label_dict[self.lbllist[i]].items():
    ...
    def variable_labels(self):
        ...
        return dict(list(zip(self.varlist, self.vlblist)))
