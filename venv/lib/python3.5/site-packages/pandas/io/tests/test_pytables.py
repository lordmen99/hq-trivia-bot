...
class TestHDFStore(unittest.TestCase):
    ...
    def test_contains(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            self.assertTrue('foo/bar' in store)
            self.assertTrue('/foo/bar' in store)
            self.assertTrue('/foo/b' not in store)
            self.assertTrue('bar' not in store)

            # GH 2694
            warnings.filterwarnings('ignore', category=tables.NaturalNameWarning)
            store['node())'] = tm.makeDataFrame()
            self.assertTrue('node())' in store)
    ...
    def test_versioning(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            store.append('df1', df[10:])
            self.assertTrue(store.root.a._v_attrs.pandas_version == '0.10.1')
            self.assertTrue(store.root.b._v_attrs.pandas_version == '0.10.1')
            self.assertTrue(store.root.df1._v_attrs.pandas_version == '0.10.1')
    ...
    def test_reopen_handle(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            self.assertTrue(store._handle.isopen)
    ...
    def test_append_some_nans(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            self.assertTrue(store.get_storer('df4').nrows == 4)
            ...
            self.assertTrue(store.get_storer('df5').nrows == 20)
            ...
            self.assertTrue(store.get_storer('df6').nrows == 20)
    ...
    def test_append_with_strings(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            def check_col(key,name,size):
                self.assertTrue(getattr(store.get_storer(key).table.description,name).itemsize == size)
        ...
        with ensure_clean(self.path) as store:
            ...
            check_col('df', 'A', 200)
            self.assertTrue(store.get_storer('df').data_columns == ['A'])

            # a min_itemsize that creates a data_column2
            _maybe_remove(store, 'df')
            store.append('df', df, data_columns = ['B'], min_itemsize={'A' : 200 })
            check_col('df', 'A', 200)
            self.assertTrue(store.get_storer('df').data_columns == ['B','A'])
            ...
            self.assertTrue(store.get_storer('df').data_columns == ['B'])
    ...
    def test_append_raise(self):
        ...
        with ensure_clean(self.path) as store:
            ...
            df = tm.makeDataFrame()
            df['invalid'] = [['a']] * len(df)
            self.assertTrue(df.dtypes['invalid'] == np.object_)
        ...
        with ensure_clean(self.path) as store:
            ...
            for attr in ['freq','tz','name']:
                for idx in ['index','columns']:
                    self.assertTrue(getattr(getattr(df,idx),attr,None) == getattr(getattr(result,idx),attr,None))
            ...
            self.assertTrue(store.get_storer('data').info['index']['freq'] is None)
    ...
    def test_retain_index_attributes2(self):
        ...
        with tm.ensure_clean(self.path) as path:
            ...
            df.to_hdf(path,'data',mode='w',append=True)
            self.assertTrue(read_hdf(path,'data').index.name == 'foo')

            idx2 = date_range('2001-1-1',periods=3,freq='H')
            idx2.name = 'bar'
            df2 = DataFrame(dict(A = Series(range(3), index=idx2)))
            df2.to_hdf(path,'data',append=True)
            self.assertTrue(read_hdf(path,'data').index.name is None)
    ...
    def test_copy(self):
        ...
        def do_copy(f = None, new_f = None, keys = None, propindexes = True, **kwargs):
            try:
                ...
                self.assertTrue(set(keys) == set(tstore.keys()))

                # check indicies & nrows
                for k in list(tstore.keys()):
                    if tstore.get_storer(k).is_table:
                        new_t = tstore.get_storer(k)
                        orig_t = store.get_storer(k)

                        self.assertTrue(orig_t.nrows == new_t.nrows)

                        # check propindixes
                        if propindexes:
                            for a in orig_t.axes:
                                if a.is_indexed:
                                    self.assertTrue(new_t[a.name].is_indexed == True)
