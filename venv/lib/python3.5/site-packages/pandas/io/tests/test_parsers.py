...
"""
        ...
        df = self.read_csv(StringIO(data), header=None,
                           ...
                           parse_dates={'nominal': [1, 2],
                                        'actual': [1, 3]})
        self.assertTrue('nominal' in df)
        self.assertTrue('actual' in df)
        self.assertTrue('X1' not in df)
        self.assertTrue('X2' not in df)
        self.assertTrue('X3' not in df)

        d = datetime(1999, 1, 27, 19, 0)
        self.assertTrue(df.ix[0, 'nominal'] == d)
        ...
        self.assertTrue(1 in df)
        self.assertTrue(2 in df)
        self.assertTrue(3 in df)
...
"""
        ...
        self.assertTrue('X1_X2' in df)
        self.assertTrue('X1_X3' in df)
        self.assertTrue('X1' not in df)
        self.assertTrue('X2' not in df)
        self.assertTrue('X3' not in df)

        d = datetime(1999, 1, 27, 19, 0)
        self.assertTrue(df.ix[0, 'X1_X2'] == d)

        df = read_csv(StringIO(data), header=None,
                      parse_dates=[[1, 2], [1, 3]], keep_date_col=True)

        self.assertTrue('1_2' in df)
        self.assertTrue('1_3' in df)
        self.assertTrue(1 in df)
        self.assertTrue(2 in df)
        self.assertTrue(3 in df)
...
KORD,19990127, 23:00:00, 22:56:00, -0.5900, 1.7100, 4.6000, 0.0000, 280.0000"""
        ...
        self.assertTrue(not isinstance(df.nominal[0], str))
...
"""
        ...
        self.assertTrue(xp.index.name == rs.index.name)
    ...
    def test_converter_index_col_bug(self):
        ...
        tm.assert_frame_equal(rs, xp)
        self.assertTrue(rs.index.name == xp.index.name)
...
"""
        ...
        try:
            ...
            self.assertTrue('Expected 3 fields in line 4, saw 5' in str(inst))
...
"""
        try:
            ...
            self.assertTrue('Expected 3 fields in line 6, saw 5' in str(inst))
...
"""
        ...
        assert_almost_equal(df.values, expected)
        self.assertTrue(np.array_equal(df.columns,
                                    ['A', 'B', 'C', 'Unnamed: 3',
                                     'Unnamed: 4']))
...
01/05/2010;P;P;50;1;15/1/2011'''
        ...
        self.assertTrue(isinstance(expected['aux_date'][0], datetime))
...
"""
        ...
        self.assertTrue(np.array_equal(df_pref.columns,
                                    ['X0', 'X1', 'X2', 'X3', 'X4']))
        self.assertTrue(np.array_equal(df.columns, list(range(5))))

        self.assertTrue(np.array_equal(df2.columns, names))
...
"""
    ...
    def test_read_csv_dataframe(self):
        ...
        df2 = self.read_table(self.csv1, sep=',', index_col=0,
                              parse_dates=True)
        self.assertTrue(np.array_equal(df.columns, ['A', 'B', 'C', 'D']))
        self.assertTrue(df.index.name == 'index')
        self.assertTrue(isinstance(df.index[0], (datetime, np.datetime64,
                                              Timestamp)))
        self.assertTrue(df.values.dtype == np.float64)
    ...
    def test_read_csv_no_index_name(self):
        ...
        df2 = self.read_table(self.csv2, sep=',', index_col=0,
                              parse_dates=True)
        self.assertTrue(np.array_equal(df.columns, ['A', 'B', 'C', 'D', 'E']))
        self.assertTrue(isinstance(df.index[0], (datetime, np.datetime64,
                                              Timestamp)))
        self.assertTrue(df.ix[:, ['A', 'B', 'C', 'D']].values.dtype == np.float64)
        tm.assert_frame_equal(df, df2)

    def test_read_table_unicode(self):
        fin = BytesIO('\u0141aski, Jan;1'.encode('utf-8'))
        df1 = read_table(fin, sep=";", encoding="utf-8", header=None)
        self.assertTrue(isinstance(df1[0].values[0], str))
...
"""
        ...
        self.assertTrue(data['A'].dtype == np.bool_)
...
"""
        data = self.read_csv(StringIO(data))
        self.assertTrue(data['A'].dtype == np.float64)
        self.assertTrue(data['B'].dtype == np.int64)
...
1,2,3"""
    ...
    def test_iterator(self):
        ...
        treader = self.read_table(StringIO(self.data1), sep=',', index_col=0,
                                  iterator=True)
        self.assertTrue(isinstance(treader, TextFileReader))
...
"""
        ...
        self.assertTrue(isinstance(df.index.levels[0][0],
                     (datetime, np.datetime64, Timestamp)))

        # specify columns out of order!
        df2 = self.read_csv(StringIO(data), index_col=[1, 0], parse_dates=True)
        self.assertTrue(isinstance(df2.index.levels[1][0],
                     (datetime, np.datetime64, Timestamp)))
...
"""
        ...
        self.assertTrue(isinstance(result['D'][0], (datetime, Timestamp)))
        tm.assert_frame_equal(result, expected)
    ...
    def test_converters_no_implicit_conv(self):
        ...
        self.assertTrue(df[0].dtype == object)
...
3;878,158;108013,434;GHI;rez;2,735694704"""
        ...
        self.assertTrue(df2['Number1'].dtype == float)
        self.assertTrue(df2['Number2'].dtype == float)
        self.assertTrue(df2['Number3'].dtype == float)
...
"""
    ...
    def test_parse_tz_aware(self):
        ...
        self.assertTrue(stamp.minute == 39)
        try:
            self.assertTrue(result.index.tz is pytz.utc)
        except AssertionError:  # hello Yaroslav
            arr = result.index.to_pydatetime()
            result = tools.to_datetime(arr, utc=True)[0]
            self.assertTrue(stamp.minute == result.minute)
            self.assertTrue(stamp.hour == result.hour)
            self.assertTrue(stamp.day == result.day)
...
KORD6,19990127, 23:00:00, 22:56:00, -0.5900, 1.7100, 4.6000, 0.0000, 280.0000"""
    ...
    def test_multiple_date_cols_chunked(self):
        ...
        self.assertTrue('nominalTime' not in df)
...
"""
        ...
        expected = self.read_csv(StringIO(re.sub('[ ]+', ',', data)),
                                 index_col=0)
        self.assertTrue(expected.index.name is None)
        tm.assert_frame_equal(df, expected)
...
two,1,2,3"""
        ...
        try:
            ...
            self.assertTrue(buf.getvalue() == 'Filled 3 NA values in column a\n')
        finally:
            sys.stdout = sys.__stdout__
...
eight,1,2,3"""
        ...
        try:
            ...
            self.assertTrue(buf.getvalue() == 'Filled 1 NA values in column a\n')
        finally:
            sys.stdout = sys.__stdout__
...
4,5.5"""
        ...
        result = self.read_csv(StringIO(data), dtype={'one': 'u1', 1: 'S1'},
                               as_recarray=True)
        self.assertTrue(result['one'].dtype == 'u1')
        self.assertTrue(result['two'].dtype == 'S1')
