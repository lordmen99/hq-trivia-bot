...
class PythonParser(ParserBase):
    ...
    def _convert_data(self, data):
        ...
        for col, f in self.converters.items():
    ...
    if na_values is None and keep_default_na:
        ...
        if keep_default_na:
            for k, v in na_values.items():
                ...
                na_values[k] = v
        na_fvalues = dict([ (k, _floatify_na_values(v)) for k, v in list(na_values.items()) ])
    else:
...
def _clean_index_names(columns, index_col):
    ...
    # hack
    if isinstance(index_names[0], str) and 'Unnamed' in index_names[0]:
        index_names[0] = None
...
def _concat_date_cols(date_cols):
    if len(date_cols) == 1:
        if py3compat.PY3:
            ...
            return np.array([str(x) if not isinstance(x, str) else x
