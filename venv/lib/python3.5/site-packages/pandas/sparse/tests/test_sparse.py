...
class TestSparseSeries(TestCase,
    ...
    def test_constructor(self):
        ...
        self.assertTrue(isinstance(self.bseries.sp_index, BlockIndex))
        self.assertTrue(np.isnan(self.iseries.fill_value))
        self.assertTrue(isinstance(self.iseries.sp_index, IntIndex))
        ...
        self.assertTrue(isinstance(s5, SparseTimeSeries))
    ...
    def test_copy_astype(self):
        ...
        self.assertTrue(cop is not self.bseries)
        self.assertTrue(cop.sp_index is self.bseries.sp_index)
        self.assertTrue(cop.dtype == np.float64)
        ...
        cop.sp_values[:5] = 97
        self.assertTrue(cop.sp_values[0] == 97)
        self.assertTrue(self.bseries.sp_values[0] != 97)
    ...
    def test_get_get_value(self):
        ...
        self.assertTrue(self.bseries.get(len(self.bseries) + 1) is None)
    ...
    def test_set_value(self):
        ...
        self.assertTrue(res is not self.btseries)
        self.assertEqual(res[idx], 0)

        res = self.iseries.set_value('foobar', 0)
        self.assertTrue(res is not self.iseries)
    ...
    def test_getitem_slice(self):
        ...
        res = self.bseries[::2]
        self.assertTrue(isinstance(res, SparseSeries))
    ...
    def test_take(self):
        def _compare_with_dense(sp):
            ...
            def _compare(idx):
                ...
                self.assertTrue(isinstance(sparse_result, SparseSeries))
    ...
    def test_reindex(self):
        ...
        self.assertTrue(same_index is not self.bseries)
    ...
    def test_sparse_reindex(self):
        ...
        def _check(values, index1, index2, fill_value):
            ...
            self.assertTrue(reindexed.sp_index is index2)
    ...
    def test_dropna(self):
        ...
        self.assertTrue(sp_valid.index.equals(sp.to_dense().valid().index))
        self.assertEqual(len(sp_valid.sp_values), 2)

        result = self.bseries.dropna()
        expected = self.bseries.to_dense().dropna()
        self.assertTrue(not isinstance(result, SparseSeries))
    ...
    def test_shift(self):
        ...
        self.assertTrue(shifted is not series)
...
class TestSparseDataFrame(TestCase, test_frame.SafeForSparse):
    ...
    def test_as_matrix(self):
        ...
        self.assertTrue(empty.shape == (0, 0))

        no_cols = SparseDataFrame(index=np.arange(10))
        mat = no_cols.as_matrix()
        self.assertTrue(mat.shape == (10, 0))

        no_index = SparseDataFrame(columns=np.arange(10))
        mat = no_index.as_matrix()
        self.assertTrue(mat.shape == (0, 10))

    def test_copy(self):
        cp = self.frame.copy()
        self.assertTrue(isinstance(cp, SparseDataFrame))
        assert_sp_frame_equal(cp, self.frame)
        self.assertTrue(cp.index is self.frame.index)

    def test_constructor(self):
        for col, series in self.frame.items():
            self.assertTrue(isinstance(series, SparseSeries))

        self.assertTrue(isinstance(self.iframe['A'].sp_index, IntIndex))
    ...
    def test_constructor_empty(self):
        sp = SparseDataFrame()
        self.assertTrue(len(sp.index) == 0)
        self.assertTrue(len(sp.columns) == 0)
    ...
    def test_constructor_from_series(self):
        ...
        self.assertTrue(isinstance(x,SparseSeries))
        df = SparseDataFrame(x)
        self.assertTrue(isinstance(df,SparseDataFrame))
    ...
    def test_dense_to_sparse(self):
        df = DataFrame({'A': [nan, nan, nan, 1, 2],
                        'B': [1, 2, nan, nan, nan]})
        sdf = df.to_sparse()
        self.assertTrue(isinstance(sdf, SparseDataFrame))
        self.assertTrue(np.isnan(sdf.default_fill_value))
        self.assertTrue(isinstance(sdf['A'].sp_index, BlockIndex))
        tm.assert_frame_equal(sdf.to_dense(), df)

        sdf = df.to_sparse(kind='integer')
        self.assertTrue(isinstance(sdf['A'].sp_index, IntIndex))
    ...
    def _check_frame_ops(self, frame):
        ...
        def _compare_to_dense(a, b, da, db, op):
            ...
            if isinstance(a, DataFrame) and isinstance(db, DataFrame):
                ...
                self.assertTrue(isinstance(mixed_result, SparseDataFrame))
    ...
    def test_op_corners(self):
        ...
        self.assertTrue(isinstance(foo.index, DatetimeIndex))
    ...
    def test_setitem(self):
        def _check_frame(frame):
            ...
            # insert SparseSeries
            frame['E'] = frame['A']
            self.assertTrue(isinstance(frame['E'], SparseSeries))
            ...
            self.assertTrue(isinstance(frame['F'], SparseSeries))
            ...
            self.assertTrue(isinstance(frame['H'], SparseSeries))
    ...
    def test_apply(self):
        ...
        self.assertTrue(isinstance(broadcasted, SparseDataFrame))
        assert_frame_equal(broadcasted.to_dense(),
                           self.frame.to_dense().apply(np.sum, broadcast=True))

        self.assertTrue(self.empty.apply(np.sqrt) is self.empty)
    ...
    def test_applymap(self):
        ...
        self.assertTrue(isinstance(result, SparseDataFrame))
    ...
    def test_stack_sparse_frame(self):
        def _check(frame):
            ...
            self.assertTrue(np.array_equal(from_dense_lp.values,
    ...
    def test_shift(self):
        def _check(frame):
            ...
            self.assertTrue(shifted is not frame)
...
class TestSparsePanel(TestCase,
    ...
    def test_pickle(self):
        def _test_roundtrip(panel):
            ...
            unpickled = pickle.loads(pickled)
            self.assertTrue(isinstance(unpickled.items, Index))
            self.assertTrue(isinstance(unpickled.major_axis, Index))
            self.assertTrue(isinstance(unpickled.minor_axis, Index))
    ...
    def test_dense_to_sparse(self):
        ...
        dwp = wp.to_sparse()
        self.assertTrue(isinstance(dwp['ItemA']['A'], SparseSeries))
    ...
    def test_to_frame(self):
        def _compare_with_dense(panel):
            ...
            self.assertTrue(np.array_equal(slp.values, dlp.values))
    ...
    def test_set_value(self):
        def _check_loc(item, major, minor, val=1.5):
            ...
            self.assertTrue(res is not self.panel)
    ...
    def test_delitem_pop(self):
        ...
        self.assertTrue(pop is crackle)
    ...
    def test_reindex(self):
        ...
        self.assertTrue('E' not in self.panel['ItemA'])
