...
def adjoin(space, *lists):
    ...
    out_lines = []
    newLists = []
    lengths = [max(list(map(len, x))) + space for x in lists[:-1]]

    # not the last one
    lengths.append(max(list(map(len, lists[-1]))))

    maxLen = max(list(map(len, lists)))
    for i, lst in enumerate(lists):
        nl = [x.ljust(lengths[i]) for x in lst]
        nl.extend([' ' * lengths[i]] * (maxLen - len(lst)))
        newLists.append(nl)
    toJoin = list(zip(*newLists))
    for lines in toJoin:
...
def is_number(obj):
    return isinstance(obj, (np.number, int, float, complex))
...
def _is_sequence(x):
    try:
        ...
        return not isinstance(x, str) and True
    except Exception:
        return False
