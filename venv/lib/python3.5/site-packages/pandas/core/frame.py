...
class DataFrame(NDFrame):
    ...
    _AXIS_NAMES = dict((v, k) for k, v in _AXIS_NUMBERS.items())
    ...
    def to_dict(self, outtype='dict'):
        ...
        if outtype.lower().startswith('d'):
            return dict((k, v.to_dict()) for k, v in self.items())
        elif outtype.lower().startswith('l'):
            return dict((k, v.tolist()) for k, v in self.items())
        ...
        if index:
            if com.is_datetime64_dtype(self.index) and convert_datetime64:
                ...
                if isinstance(self.index, MultiIndex):
                    ...
                    ix_vals = list(map(np.array,list(zip(*self.index.values))))
                else:
    ...
    def info(self, verbose=True, buf=None, max_cols=None):
        ...
        dtypes = ['%s(%d)' % k for k in sorted(counts.items())]
    ...
    def filter(self, items=None, like=None, regex=None):
        ...
        if items is not None:
            ...
            matchf = lambda x: (like in x if isinstance(x, str)
                                else like in str(x))
    ...
    def duplicated(self, cols=None, take_last=False):
        ...
        if cols is None:
            ...
            if np.iterable(cols) and not isinstance(cols, str):
    ...
    def describe(self, percentile_width=50):
        ...
        return self._constructor(list(map(list, list(zip(*destat)))), index=destat_columns,
...
def _rec_to_dict(arr):
    if isinstance(arr, np.ndarray):
        ...
        sdict = dict((k, v.values) for k, v in arr.items())
