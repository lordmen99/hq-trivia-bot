...
def _get_grouper(obj, key=None, axis=0, level=None, sort=True):
    ...
    any_callable = any(isinstance(g, collections.Callable) or isinstance(g, dict) for g in keys)
...
def _is_label_like(val):
    return isinstance(val, str) or np.isscalar(val)
...
class NDFrameGroupBy(GroupBy):
    ...
    def aggregate(self, arg, *args, **kwargs):
        ...
        if isinstance(arg, dict):
            ...
            if any(isinstance(x, (list, tuple, dict)) for x in list(arg.values())):
                new_arg = OrderedDict()
        ...
        if not self.as_index:
            if isinstance(result.index, MultiIndex):
                zipped = list(zip(result.index.levels, result.index.labels,
                             result.index.names))
...
class DataFrameGroupBy(NDFrameGroupBy):
    ...
    def _wrap_aggregated_output(self, output, names=None):
        ...
        if not self.as_index:
            ...
            zipped = list(zip(self.grouper.names, group_levels))
