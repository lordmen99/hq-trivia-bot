...
class Series(generic.PandasContainer, pa.Array):
    ...
    def __unicode__(self):
        ...
        if len(self.index) > (max_rows or 1000):
            ...
            result = 'Series([], dtype: %s)' % self.dtype
    ...
    def _repr_footer(self):
        namestr = "Name: %s, " % com.pprint_thing(
            self.name) if self.name is not None else ""
        return '%sLength: %d, dtype: %s' % (namestr, len(self),
                                             str(self.dtype.name))
...
# Combination
    ...
    def replace(self, to_replace, value=None, method='pad', inplace=False,
        ...
        def _rep_dict(rs, to_rep):  # replace {[src] -> dest}
            ...
            for s, d in to_rep.items():
                dd.setdefault(d, []).append(s)
                all_src.add(s)

            if any(d in all_src for d in list(dd.keys())):
                # don't clobber each other at the cost of temporaries
                masks = {}
                for d, sset in dd.items():  # now replace by each dest
        ...
        if isinstance(to_replace, dict):
            ...
            if isinstance(value, (list, pa.Array)):  # check same length
                ...
                if vl == rl:
                    _rep_dict(result, dict(list(zip(to_replace, value))))
                else:
