...
def randu(n):
    choices = "".join(map(chr, list(range(1488, 1488 + 26)))) + string.digits
    return ''.join([random.choice(choices) for _ in range(n)])
...
def assert_almost_equal(a, b, check_less_precise = False):
    ...
    if isiterable(a):
        np.testing.assertTrue(isiterable(b))
        na, nb = len(a), len(b)
    ...
    if isnull(a):
        np.testing.assertTrue(isnull(b))
        return
...
def assert_dict_equal(a, b, compare_keys=True):
    a_keys = frozenset(list(a.keys()))
    b_keys = frozenset(list(b.keys()))
...
def network(t, raise_on_error=_RAISE_NETWORK_ERROR_DEFAULT,
    ...
    A test can be decorated as requiring network like this::
      ...
      >>> @network
      ... def test_network():
      ...   urllib.request.urlopen("rabbit://bonanza.com")
      ... 
      >>> try:
      ...   test_network()
    ...
    the error to bubble up, e.g.::
      ...
      ... def test_network():
      ...   urllib.request.urlopen("complaint://deadparrot.com")
      ... 
      >>> test_network()
