...
class TestNDFrame(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertTrue(ndf.values.dtype == np.int64)

    def test_ndim(self):
        self.assertEqual(self.ndf.ndim, 2)

    def test_astype(self):
        casted = self.ndf.astype(int)
        self.assertTrue(casted.values.dtype == np.int_)

        casted = self.ndf.astype(np.int32)
        self.assertTrue(casted.values.dtype == np.int32)
