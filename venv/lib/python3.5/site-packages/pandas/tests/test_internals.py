...
class TestBlock(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertTrue(int32block.dtype == np.int32)
    ...
    def test_attrs(self):
        self.assertTrue(self.fblock.shape == self.fblock.values.shape)
        self.assertTrue(self.fblock.dtype == self.fblock.values.dtype)
        self.assertTrue(len(self.fblock) == len(self.fblock.values))
    ...
    def test_copy(self):
        ...
        self.assertTrue(cop is not self.fblock)
        assert_block_equal(self.fblock, cop)

    def test_items(self):
        cols = self.fblock.items
        self.assertTrue(np.array_equal(cols, ['a', 'c', 'e']))

        cols2 = self.fblock.items
        self.assertTrue(cols is cols2)

    def test_assign_ref_items(self):
        new_cols = Index(['foo', 'bar', 'baz', 'quux', 'hi'])
        self.fblock.set_ref_items(new_cols)
        self.assertTrue(np.array_equal(self.fblock.items,
                                    ['foo', 'baz', 'hi']))
    ...
    def test_reindex_items_from(self):
        ...
        self.assertEqual(reindexed.values.shape[0], 2)
        self.assertTrue((reindexed.values[0] == 2).all())
        self.assertTrue((reindexed.values[1] == 1).all())
...
class TestBlockManager(unittest.TestCase):
    ...
    def test_attrs(self):
        self.assertEqual(self.mgr.nblocks, len(self.mgr.blocks))
        self.assertEqual(len(self.mgr), len(self.mgr.items))

    def test_is_mixed_dtype(self):
        self.assertTrue(self.mgr.is_mixed_type)

        mgr = create_blockmanager([get_bool_ex(['a']), get_bool_ex(['b'])])
        self.assertTrue(not mgr.is_mixed_type)
    ...
    def test_is_indexed_like(self):
        ...
        self.assertTrue(not self.mgr._is_indexed_like(mgr2))
    ...
    def test_contains(self):
        self.assertTrue('a' in self.mgr)
        self.assertTrue('baz' not in self.mgr)
    ...
    def test_pickle(self):
        ...
        # share ref_items
        self.assertTrue(mgr2.blocks[0].ref_items is mgr2.blocks[1].ref_items)
    ...
    def test_set_change_dtype(self):
        ...
        self.assertTrue(self.mgr.get('baz').dtype == np.object_)

        mgr2 = self.mgr.consolidate()
        mgr2.set('baz', np.repeat('foo', N))
        self.assertTrue(mgr2.get('baz').dtype == np.object_)

        mgr2.set('quux', randn(N).astype(int))
        self.assertTrue(mgr2.get('quux').dtype == np.int_)

        mgr2.set('quux', randn(N))
        self.assertTrue(mgr2.get('quux').dtype == np.float_)

    def test_copy(self):
        shallow = self.mgr.copy(deep=False)

        for cp_blk, blk in zip(shallow.blocks, self.mgr.blocks):
            self.assertTrue(cp_blk.values is blk.values)
    ...
    def test_as_matrix_float(self):
        ...
        self.assertTrue(mgr.as_matrix().dtype == np.float64)

        mgr = create_blockmanager([get_float_ex(['c'],np.float32), get_float_ex(['d'],np.float16)])
        self.assertTrue(mgr.as_matrix().dtype == np.float32)

    def test_as_matrix_int_bool(self):

        mgr = create_blockmanager([get_bool_ex(['a']), get_bool_ex(['b'])])
        self.assertTrue(mgr.as_matrix().dtype == np.bool_)
    ...
    def test_astype(self):
        ...
        for t in ['float16','float32','float64','int32','int64']:
            tmgr = mgr.astype(t)
            self.assertTrue(tmgr.as_matrix().dtype == np.dtype(t))
    ...
    def test_convert(self):
        ...
        def _compare(old_mgr, new_mgr):
            ...
            self.assertTrue(len(old_blocks) == len(new_blocks))
            ...
            for b in new_mgr.blocks:
                if isinstance(b,block_type):
                    for i in list(b.items):
                        items.add(i)
            self.assertTrue(items == set(citems))
    ...
    def test_missing_unicode_key(self):
        df = DataFrame({"a": [1]})
        try:
            df.ix[:, "\u05d0"]  # should not raise UnicodeEncodeError
        except KeyError:
