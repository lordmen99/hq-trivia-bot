...
class TestCut(unittest.TestCase):
    ...
    def test_labels(self):
        ...
        ex_levels = ['(-0.001, 0.25]', '(0.25, 0.5]', '(0.5, 0.75]',
                     '(0.75, 1]']
        self.assertTrue(np.array_equal(result.levels, ex_levels))
    ...
    def test_qcut(self):
        ...
        self.assertTrue(np.array_equal(labels, ex_levels))

    def test_qcut_bounds(self):
        arr = np.random.randn(1000)

        factor = qcut(arr, 10, labels=False)
        self.assertTrue(len(np.unique(factor)) == 10)
    ...
    def test_qcut_binning_issues(self):
        ...
        for (sp, sn), (ep, en) in zip(list(zip(starts[:-1], starts[1:])),
                                      list(zip(ends[:-1], ends[1:]))):
